...
bool warshall(vector<vector<bool>>&);
void isTransitive(vector<vector<bool>>&);
int CountTranRelation(int);
void count();//计数函数
void judge();//判断矩阵对应关系是否传递
...
bool warshall(vector<vector<bool>>& rmatrix) {
	int n = rmatrix.size();
...
void count(){//计算n元集合上的传递关系数
	int n,count;
	cout<<"Enter the order of the set:"<<endl;
	cin>>n;
	count=CountTranRelation(n);
    cout<<"the number of transitive relations on an "<<n<<"-element set is:"<<count<<endl;

}
void judge(){//根据输入的任意n阶0~1矩阵，判断该矩阵对应的关系是否是满足传递性的
	...
	for(int i=0;i<n;i++){//读取数据,按照行优先读取,每个数据间空格隔开
		for(int j=0;j<n;j++){
			int value;
			cin>>value;
			matrix[i][j]=value;
